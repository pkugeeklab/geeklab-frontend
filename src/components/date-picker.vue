...
<template>
  <div class="cov-vue-date" :class="option.wrapperClass ? option.wrapperClass : {}">
    ...
    <div class="datepicker-overlay" v-if="showInfo.check" @click.prevent="dismiss($event)" v-bind:style="{'background' : option.overlayOpacity? 'rgba(0,0,0,'+option.overlayOpacity+')' : 'rgba(0,0,0,0.5)'}">
      <div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
        <div class="cov-date-monthly">
          <div class="cov-date-previous" @click.prevent="nextMonth('pre')">«</div>
          ...
          <div class="cov-date-next" @click.prevent="nextMonth('next')">»</div>
        </div>
        ...
        <div class="button-box" v-if="option.type==='min'">
          <span @click.prevent="showInfo.check=false">{{option.buttons? option.buttons.cancel : 'Cancel' }}</span>
          <span @click.prevent="picked">{{option.buttons? option.buttons.ok : 'Ok'}}</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    nextMonth(type) {
      let next = type === 'next' ? moment(this.checked.currentMoment).add(1, 'M') : moment(this.checked.currentMoment).add(-1, 'M')
      this.showDay(next)
    },
    showDay(time) {
